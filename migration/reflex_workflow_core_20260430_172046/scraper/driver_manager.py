...
LOW_MEMORY_HEADLESS = os.getenv("BF_HEADLESS_LOW_MEMORY", "1").strip().lower() not in {"0", "false", "no"}

def setup_driver(initial_download_dir=None, headless_mode: bool = False):
    """Setup and return a configured ChromeDriver instance."""
    try:
        logger.info(f"Setting up Chrome WebDriver (headless={bool(headless_mode)})...")
        ...
        if headless_mode:
            ...
            if LOW_MEMORY_HEADLESS:
                # Keep memory footprint lower in headless runs.
                ...
                logger.info("Applied low-memory headless Chrome profile (BF_HEADLESS_LOW_MEMORY=1)")
